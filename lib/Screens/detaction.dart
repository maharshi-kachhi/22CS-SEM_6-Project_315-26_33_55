  ...
  Future<void> runModel(String inputText) async {
    try {
      ...
      List<double> inputTensor =
          List.filled(inputSize, 0.0); // Ensure fixed size
      ...
      var outputTensor =
          List.generate(1, (_) => List.filled(3, 0.0)); // Output shape [1,3]
  ...
  void _detectHateSpeech() {
    ...
    setState(() {
      ...
      _messages.add(Message(
          text: "Processing...", isUser: false)); // Show processing message
    });
    ...
    Future.delayed(const Duration(seconds: 1), () {
      setState(() {
        ...
        _messages.removeWhere((msg) => msg.text == "Processing...");

        if (_modelOutput.isEmpty || _modelOutput[0].isEmpty) {
          _messages.add(Message(
              text: "Try again for '$inputText' later.", isUser: false));
          return;
        }
